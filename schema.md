...
| `% ... >` | Template inheritance (shorthand) | `% audit > base` |
...
| `[...]` | CHECK constraint | any | `status 1 =0 [0,1,2]` |
...
|--------|-------------|---------|
| `[CASCADE]` | `[C]` | `-> uid user.id [C]` |
| `[SET NULL]` | `[SN]` | `-> uid user.id [SN]` |
| `[NO ACTION]` | `[NA]` | `-> uid user.id [NA]` |
| `[RESTRICT]` | `[R]` | `-> uid user.id [R]` |
| `[CASCADE, UPDATE CASCADE]` | `[C, U C]` | `-> uid user.id [C, U C]` |
...
| `@u name` | Single-column UNIQUE INDEX | `@u email` → `UNIQUE INDEX uk_email (email)` |
...
> **Tip**: Bare `s` is equivalent to omitting the type entirely (both produce `varchar`). Omitting saves 1 keystroke — use explicit `s` only when readability requires it.
...
Marks a single column as primary key (without auto increment). Can be fused with the type symbol:

```asm
code s32 !     ; separate tokens: varchar(32) PRIMARY KEY
code s32!      ; fused: same result, saves 1 char
user_id n!     ; fused: int PRIMARY KEY (composite PK)
```
...
**Ultra-shorthand** — infer local field from reference (saves 7+ chars per FK):

```asm
-> user.id [CASCADE]        ; infers local field as user_id
-> user.id                  ; bare: infers field, no action
```

When the reference is `table.field` and no local field name is provided, the compiler infers `{table}_id` as the local field. This covers 89% of real-world FKs where the convention is `X_id → X.id`.

> **Tip**: To document FK intent without generating DDL, use a spec comment: `user_id ; -> user.id`. This records the relationship in source but doesn't create a database constraint.

### FK Actions

Append `[action]` to specify referential actions. Supported actions:

| Action | Abbreviation | SQL Output |
|--------|-------------|-----------|
| `CASCADE` | `C` | ON DELETE CASCADE |
| `SET NULL` | `SN` | ON DELETE SET NULL |
| `NO ACTION` | `NA` | ON DELETE NO ACTION (default) |
| `RESTRICT` | `R` | ON DELETE RESTRICT |
| `UPDATE <action>` | `U <abbrev>` | ON UPDATE instead of ON DELETE |

Multiple actions are comma-separated: `[CASCADE, UPDATE SET NULL]` or `[C, U SN]`.
...
@u unique_name (field1, field2)     ; unique index
...
- Unique fulltext index (`@fu`): MySQL does not allow UNIQUE on FULLTEXT indexes
...
Fields are nullable by default. Mark with `*` for NOT NULL. Can be fused with the type symbol:

```asm
name    s32 *    ; separate tokens: varchar(32) NOT NULL
name    s32*     ; fused: same result, saves 1 char
email   s128*    ; fused: varchar(128) NOT NULL
```
...
```asm
status  1 =0 [0,1,2]         ; CHECK (status IN (0, 1, 2))
...
Templates can extend other templates using `extends` or the shorter `>` shorthand:
...
% audit extends base    ; full syntax
% audit > base          ; shorthand (saves 6 chars)
...
The compiler emits warnings for common mistakes:

**Unrecognized tokens** — When a token in a field declaration doesn't match any known pattern:

```
warning: unrecognized token 'foo' in field 'bar' (line 5)
```

This typically means a typo in a modifier or type symbol. Check the token against the modifier list in §4.

**Invalid modifier** — When a modifier is used with an incompatible type:

```
warning: 'auto_increment' modifier invalid for this type (line 3) — valid on: n, N, integer types, t, d
```

`+` and `++` only work on numeric types (`n`, `N`, `\d+`) for AUTO_INCREMENT and datetime types (`t`, `d`) for timestamp defaults. Use explicit `!` or `=` for other types.

**Type-modifier fusion** — The parser accepts fused forms like `n!`, `n*`, `s32!`. These are equivalent to separate tokens (`n !`, `n *`, `s32 !`) but save 1 keystroke per field.

---

## 14. Design Principles

1. **Minimum keystrokes** — the core motivation. Every design decision optimizes for typing less: single-character types (`n`, `s`, `m`), fused modifiers (`n!`, `n*`), shorthand indexes (`@ field`), FK abbreviations (`[C]`), and suffix inference that eliminates explicit types.
2. **Minimal syntax** — every construct is a single character or short symbol. No keywords, no braces, no quotes around names.
3. **Convention over configuration** — suffix inference (`_id`, `_on`, `_at`) and sensible defaults (no type = varchar) eliminate redundancy.
4. **Shorthand where unambiguous** — single-column indexes (`@ field`) and FKs (`-> field table.field`) omit redundant names/brackets when the context is clear.
5. **Type Spec as foundation** — field types are fully delegated to [Type Spec](type.md); schema.md only adds structure and constraints.
6. **Modifier composition** — symbols like `++` compose primitive modifiers (`+` + `!` for numeric, `+` + `+` for timestamp) into common patterns.
7. **Three-layer comments** — spec (`;`) for authoring notes, SQL (`--`) for DDL comments, column (`//`) for COMMENT clauses.
8. **Template-driven** — define table patterns once, apply them everywhere. The `...` slot gives precise control over field ordering.
9. **DB-agnostic core** — symbols map to SQL standard concepts; the consuming tool handles dialect-specific DDL (MySQL, PostgreSQL, etc.).
...
This adds `deleted_at` and `deleted_by` (with explicit `n` type) fields to every table using the template.
...
# Generate DDL for v1
./zig-out/bin/typespec ../schema_v1.tps -o schema_v1.sql

# Make changes to schema_v1.tps → schema_v2.tps
./zig-out/bin/typespec ../schema_v2.tps -o schema_v2.sql

# Diff the SQL output manually or with your DBA tools
