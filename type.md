...
> **Note**: Bare `s` is equivalent to omitting the type entirely — both produce `varchar`. Omitting is preferred (saves 1 keystroke). Use explicit `s` only when readability requires it.
...
The parser recognizes `!` and `*` suffixes on type tokens, in addition to the existing `+` and `++` handling.
...
|------------|-----------|
| `type_symbol` | `numeric_type \| money_type \| string_type \| atomic_type` |
