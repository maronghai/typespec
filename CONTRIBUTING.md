...
This project uses **golden file testing** — each `.tps` example has a corresponding `.sql` file with expected output.

1. Build the compiler:
   ```bash
   cd zig-typespec && zig build
   ```
2. Generate output for an example:
   ```bash
   ./zig-out/bin/typespec ../examples/user-order.tps
   ```
3. Compare with the golden file:
   ```bash
   diff <(./zig-out/bin/typespec ../examples/user-order.tps) ../examples/user-order.sql
   ```
4. If your change affects output, update the `.sql` golden file to match.
5. For new features, add a `.tps` + `.sql` pair in `examples/`.
