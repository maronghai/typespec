...
pub const DEFAULT_PASSES = [_]SemanticPass{
    ...
    .{ .name = "validate_schema", .run = runValidateSchema, .depends_on = &.{"validate"} },
};
...
/// self-referencing FK field count mismatch. Runs after validate pass.
fn runValidateSchema(ctx: *PassContext) !void {
    // 1. Build table name → fields index for FK target field checking
    var table_map = std.StringHashMap(*const ResolvedTable).init(ctx.alloc);
    for (ctx.tables.items) |*t| {
        try table_map.put(t.name, t);
    }

    // 2. Build FK graph for cycle detection (table_name → list of ref_table names)
    ...
    // 3. DFS cycle detection (warning — not blocking)
    ...
    // 4. FK target field existence check (error)
    for (ctx.tables.items) |table| {
        for (table.fks) |fk| {
    ...
    // 5. Self-referencing FK field count validation (error)
    for (ctx.tables.items) |table| {
        for (table.fks) |fk| {
...
/// Reports circular FK chains as warnings.
fn detectCycle(
    ctx: *PassContext,
    ...
    if (fk_graph.get(node)) |refs| {
        for (refs.items) |ref_table| {
            if (visited.get(ref_table)) |status| {
                if (status == .visiting) {
                    // Found cycle — build error message with cycle path
...
fn validateFkTargetFields(
    ...
    const target = table_map.get(fk.ref_table) orelse return; // FK target table existence is checked in validate pass
